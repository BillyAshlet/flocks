      ...
      sections: [
        {
          ...
          formulas: [
            `R_c = \\max\\!\\left(\\sqrt[3]{\\dfrac{3\\,${K}\\,V}{4\\pi\\,${N}}},\\; 0.138\\right),\\qquad R_s = ${FS}\\,R_c,\\qquad R_a = ${FA}\\,R_c`,
          ],
        ...
        },
        {
          ...
          text: [
            'Separation pushes away from close neighbors, each one weighted by the inverse of its distance. Alignment is the average velocity of the neighbors. Cohesion points at a weighted center of the neighbors, where close fish count much more than far ones, so a fish belongs to its own pod before the school as a whole.',
          ],
          formulas: [
            `\\mathbf{s}_i = -\\!\\!\\sum_{d_{ij} < R_s}\\! \\frac{\\mathbf{x}_j - \\mathbf{x}_i}{d_{ij}^{2}}`,
        ...
        },
        {
          ...
          formulas: [
            ...
            `\\mathbf{F}^{\\text{home}}_i = ${WRC}\\,\\operatorname{steer}(-\\mathbf{x}_i),\\quad \\text{on from } ${TDELAY} \\text{ to } ${TDELAY} + ${TPULL} \\text{ after a hit}`,
          ],
